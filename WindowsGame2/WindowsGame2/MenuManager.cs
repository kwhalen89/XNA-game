...
namespace WindowsGame2
{
    public class MenuManager
    {
        ...
        private void SetAnimations()
        {
            ...
            if (align.Contains("Center")) //if align is set to center
            { //then calculates the total width and height of all menu items combined in order to Center them accordingly
                for (int i = 0; i < menuItems.Count; i++)
                {
                ...
                if (axis == 1) //if on axis 1 subtract dimensions of menu by actual screen dimensions and divide by 2
                {
            ...
            else //or else if not specified to be Centered then just set to position
            { 
                pos = position;
            }
            ...
            for (int i = 0; i < menuImages.Count; i++)
            {
                ...
                for (int j = 0; j < animationTypes.Count; j++)
                {
                    ...
                    //in order to make dimensions calculations correct for whatever font used
                }
                if (axis == 1)
                {
                    pos.X += dimensions.X;
